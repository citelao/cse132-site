...
Pages 1 and 3 of the spec sheet have all the information you need to complete this assignment and cover sheet.

### The TMP36

The temperature sensor we are using, the TMP36, operates fairly simply. It connects to your circuits about the same way as a potentiometer. When a voltage passes across the two outer pins (in the right direction), the center pin outputs a voltage based on the temperature. 

![A thermistor with pins labeled](../img/tempsensor.png)

You must wire the temperature sensor in the correct way or it will get hot. With the pins *facing* you and the flat end pointed up, the pin on the far left is high, and the pin on the far right is ground. Attach the high pin to your power source and the ground pin to ground.

<aside class="warning">
The temperature sensor *will* burn you if you wire it incorrectly.
</aside>

The output voltage is independent of the input voltage: it is a linear function based on a temperature. At $25^{\circ} C$, the TMP36 outputs a specific voltage (written in the spec sheet), and as the temperature changes, the output voltage changes based on a **scale factor** ($mV/ ^{\circ}  C$, also in the spec sheet).
...
In order to read the temperature output by our sensor, you need to read the voltage into your Arduino. You have experience with this: you used `analogRead()` to read a voltage from a potentiometer in an earlier studio.
...
One option is to build physical hardware to filter: the [RC filter](http://www.electronics-tutorials.ws/filter/filter_2.html) is one such filter, and would do fairly nicely if this were an electical engineering class. But we want to handle this in software, so we will use a very simple, [pretty bad](http://www.analog.com/media/en/technical-documentation/dsp-book/dsp_book_Ch15.pdf) filter: a **rolling average**.

A rolling average filter takes a new data point, finds the mean of the $N$ most recent data points for some $N$, and outputs that as the true data point instead.

In mathematical speak:

$$ y_i = \frac{1}{N} \sum\limits_{j=0}^{N} x_{i-j} $$

It is only possible to do this if you store the $N$ most recent data points. No other method can produce the same output.

And the easiest way to do this is with an **array**.

#### Arrays

Though the syntax is very similar to Java's, C arrays and Java arrays are noticeably different. Though they both are **zero-indexed** and accessed with square brackets (`puppies[0]` gives the *first* puppy in the array while `puppies[1]` gives the second), the declaration for each is slightly different and C does *not* **bounds check**.

C arrays do not prevent you from accessing invalid indexes, like `6` for an array of size `3`.
...
1. Attach an LED to the pin of your choice, add a resistor, and attach it to ground. Make sure you can turn it on and off.
2. Connect the center, output stem of your temperature sensor to an analog pin. Then attach the source stem to `+5V` and the ground stem to `GND`. If you hold the stems of your temperature sensor towards you and point the flat notch up, the source stem is on the left and the ground stem is on the right.

	Make sure you can `analogRead()` from it, even if you haven't set `analogReference()` yet.
3. Write a simple delta time loop to `analogRead()` the temperature at around 40Hz (40 times a second). For now, just `Serial.print()` it out, without any conversion.
3. Set `analogReference()` to `INTERNAL`. This will change what each value of `analogRead()` corresponds to (`1023` will now correspond to $1.1V$, not $5V$), which will be important when you...
4. Convert the raw `analogRead()` value into a temperature, first by converting the raw value into a voltage, then into a temperature from the [spec sheet for the TMP36](http://www.analog.com/media/en/technical-documentation/data-sheets/TMP35_36_37.pdf).

	The spec sheet provides a **scale factor** ($\frac{mV}{C}$) of how much output voltage changes for every degree Celsius and a base voltage that is output at $25^{\circ} C$. With that data point and the slope, you can create an equation that transforms voltage into temperature and vice versa.
	
	Print both out until you are confident that your temperature is right.
...
2. Then create an array of size `FILTER_COUNTS` to store the `N` most recent data points. The simplest way to do this is by maintaining a count of the number of reads you do: whenever you read a temperature, save it to this array at index `count % FILTER_COUNTS`.
...
3. Print this rolling average alongside the rolling temperature. Is it more stable? Does it still respond to temperature changes?
4. Graph the temperature data (unfiltered and filterred) you get from your Arduino in your favorite graphing program. If you structure your output something like this, with commas separating each field of the data:
	...
	Show a noticeable rise in temperature by holding the temperature sensor and letting go. Make sure to graph both the raw values and the filtered values. Also, double check your temperatures to make sure they're reasonable.
...
1. [Dolbear's Law](https://en.wikipedia.org/wiki/Dolbear%27s_law) is a formula to calculate the temperature from the amount of cricket chirps in 60 seconds:
	...
	Solve this equation to determine a period, a number that you can use in a separate delta time loop to flash a cricket LED.
2. However, even with this number, flashing the LED is not that simple. You need to keep track of whether or not your LED is on (like with a [`boolean`](https://www.arduino.cc/en/Reference/BooleanVariables)), and then alternate between *two* intervals for this "flash" delta time loop:
	...
	Write the appropriate delta time loop (separate from your temperature sampling code) to blink the LED based on temperature. Your program should now asynchronously measure temeperature and output data to your PC while blinking the LED.
...
1. Make sure to wire the temperature sensor correctly or you will burn yourself.
	...
	The stems number from 1 to 3 left to right with the flat side up and the stems facing you. 
	
	1. Voltage
	2. Output
	3. Ground
2. Your final circuit should look like this:
...
4. The easiest way to check your equations by plugging in some test values: is the output sensible at 1V? 25 Celsius? Am I blinking at the right rate? Plug at least two values in to make sure everything's working.
5. Verify all your numbers again. Graph them. Are they changing at the as you hold your temperature sensor? Is the average floating between the noise of your signal?
